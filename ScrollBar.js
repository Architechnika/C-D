...
function ScrollBar(posX,posY,orientation,arr, name)
{
  ...
  this.scrollToEnd = function()
  {
    if(arrayForBar !== undefined)
     {
      var BGRx = backGround.getBackGround().x+backGround.getBackGround().w;
      var lastItemRx = arrayForBar[0].x + arrayForBar[0].w;
      var dw = 0;
  ...
  this.sortElements = function(array, scrollBackGround)
  {
    ...
    OOP.forArr(sortArr,function(el,i)
    {
      ...
      if(orientation == "Vertical")
      {
        ...
        oldY+=itemHW;
        if (lineCount > 1) {
            if ((scrollBackGround.h + (rowCount * itemHW) + 3) < height / 100 * 85)
